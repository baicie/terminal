...
> 当前实现（2026-08-17）：移动端使用紧凑页面顶栏 + 终端专用会话栏 + 底部主导航；会话栏支持横向滚动、切换、关闭和新建，终端保留最多双 pane，完整操作通过 shadcn Sheet 和长按菜单承载。
...
桌面端顶栏复杂按钮（新建标签、SFTP 切换）在移动端简化为页面顶栏；终端路由在其下方追加独立会话栏：
...
| 桌面端 | 移动端改造 |
|--------|-----------|
| 顶栏工具栏（字体、放大按钮） | pane header 保留状态与更多入口；底部键盘栏提供字号、清屏、全屏和快捷键帮助 |
| 右键菜单 | 长按弹出上下文菜单（替代右键） |
| 标签页 | 顶栏下方 44px 会话栏显示，多会话时左右滑动并可关闭 |
| 分屏模式 | 保留最多双 pane；通过会话标签的长按/上下文菜单创建和退出分屏 |
| 键盘 | 调用系统键盘；终端底部快捷栏：Esc / Tab / Ctrl / ↑↓ / 清屏 / 全屏 |

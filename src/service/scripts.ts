...
export class ScriptService {
  ...
  private scheduleIntervalIds: Map<string, ReturnType<typeof setInterval>> = new Map();
  ...
  async executeOnHosts(
    ...
    await updateScriptExecution(executionId, {
      status: failedCount === 0 ? "success" : failedCount === successCount ? "failed" : "success",
  ...
  async executeSingleHost(host: Host, command: string, _timeoutMs = 60000): Promise<SSHOutput> {
  ...
  startSchedule(script: ScriptRecord): void {
    ...
    switch (script.schedule_type) {
      case "once": {
        const delay = script.schedule_value ? parseInt(script.schedule_value, 10) : 0;
        const timeoutId = setTimeout(() => {
          this.executeScript(script.id);
        }, delay);
        this.scheduleIntervalIds.set(script.id, timeoutId as unknown as ReturnType<typeof setInterval>);
        break;
      }
      case "interval": {
        const intervalMs = script.schedule_value ? parseInt(script.schedule_value, 10) : 60000;
  ...
  async getExecutions(scriptId?: string, limit = 100): Promise<ScriptExecutionRecord[]> {

...
const TerminalContainer: React.FC<TerminalContainerProps> = observer(
  ({ tabId }) => {
    ...
    // Buffered input for local/serial to batch rapid keystrokes
    const localInputBufferRef = useRef('')
    const localInputTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
      null,
    )
    ...
    const connectSSH = useCallback(async (host: Host) => {
      ...
      termRef.current?.write(
        '\x1b[33mConnecting to ${host.hostname}...\x1b[0m\r\n',
      )

      const connResult = await sshService.connect(host)
      if (!connResult.success || !connResult.sessionId) {
        termRef.current?.write(
          `\x1b[31mConnection failed: ${connResult.message}\x1b[0m\r\n`,
        )
      ...
      const shellResult = await sshService.startShell(
        connResult.sessionId,
        80,
        24,
        {
          id: host.id,
      ...
      if (!shellResult.success) {
        termRef.current?.write(
          `\x1b[31mShell failed: ${shellResult.message}\x1b[0m\r\n`,
        )
        setStatus('disconnected')
        return
      }
    ...
    const connectSerial = useCallback(
      async (port: string, baudRate: number) => {
        setStatus('connecting')
        termRef.current?.write(
          `\x1b[33mConnecting to ${port} at ${baudRate} baud...\x1b[0m\r\n`,
        )
    ...
    // Flush buffered local/serial input to backend
    const flushLocalInput = useCallback(async () => {
      const buf = localInputBufferRef.current
      localInputBufferRef.current = ''
      if (localInputTimerRef.current !== null) {
        clearTimeout(localInputTimerRef.current)
        localInputTimerRef.current = null
      }
      const sessionId = sessionIdRef.current
      const connType = connectionTypeRef.current
      if (!sessionId || !connType || !buf) return
      if (connType === 'local') {
        await sshService.writeLocal(sessionId, buf)
      } else if (connType === 'serial') {
        await serialService.write(sessionId, buf)
      }
    }, [])
    ...
    const handleData = useCallback(
      async (data: string) => {
        ...
        // For local PTY and serial: buffer keystrokes and flush in batches.
        // Rapid input generates many onData events; batching avoids Tauri IPC queue overflow.
        // Enter/arrow/ctrl keys are flushed immediately for responsiveness.
        if (connType === 'local' || connType === 'serial') {
          // Immediately flush control chars so Ctrl+C, arrow keys feel instant
          if (
            data.length === 1 &&
            (data < ' ' || data === '\x7f' || data >= '\x80')
          ) {
            await flushLocalInput()
            if (connType === 'local') {
          ...
          // Accumulate printable chars into buffer, flush after 10ms of silence
          localInputBufferRef.current += data
          if (localInputTimerRef.current !== null) {
            clearTimeout(localInputTimerRef.current)
          }
          localInputTimerRef.current = setTimeout(() => {
            void flushLocalInput()
          }, 10)
          return
        }
        ...
        if (data.length === 1 && code >= 32) {
          ...
          // For rapid input: fire-and-forget, don't await.
          if (connType === 'remote' && sessionId) {
            void sshService.write(sessionId, data)
          }
        }
      },
      [app.tabs, tabId, flushLocalInput],
    )
    ...
    useEffect(() => {
      ...
      const setupListeners = async () => {
        ...
        if (tab.type === 'remote') {
          ...
          unlistenClose = await sshService.onLocalClose(

...
const ViewToolbar: React.FC<ViewToolbarProps> = ({ children, className }) => {
  return (
    <div className={cn("flex items-center justify-between px-4 py-3 border-b border-border/60", className)}>
      {children}
    </div>
  );
};
...
const ViewHeader: React.FC<ViewHeaderProps> = ({ title, description, actions, className }) => {
  return (
...
const EmptyState: React.FC<EmptyStateProps> = ({ icon, title, description, action, className }) => {
  return (
    <div className={cn("flex flex-col items-center justify-center h-full py-12 px-6 text-center", className)}>
      {icon && (

...
const CommandPalette: React.FC<CommandPaletteProps> = observer(({ open, onClose }) => {
  const [query, setQuery] = useState("");
  const [results, setResults] = useState<SearchResult[]>([]);
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [activeTab, setActiveTab] = useState<"all" | "hosts" | "snippets" | "history" | "actions">("all");
  ...
  const searchHosts = useCallback((searchQuery: string): SearchResult[] => {
    if (!searchQuery) {
  ...
  // Search snippets
  const searchSnippetsAction = useCallback(async (searchQuery: string): Promise<SearchResult[]> => {
    if (!searchQuery) {
      return [];
    }
    try {
  ...
  const searchHistory = useCallback(async (searchQuery: string): Promise<SearchResult[]> => {
    if (!searchQuery) {
      return [];
    }
    try {
      ...
      const filtered = history
        .filter((h) => h.command.toLowerCase().includes(searchQuery.toLowerCase()))
        .slice(0, 10);
  ...
  }, [query, activeTab, searchHosts, searchSnippetsAction, searchHistory, quickActions]);
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      switch (e.key) {
        ...
        case "Tab":
          e.preventDefault();
          const tabs: Array<typeof activeTab> = ["all", "hosts", "snippets", "history", "actions"];
  ...
  const tabs = [
    { key: "all" as const, label: "All", icon: <Search className="h-3 w-3" /> },
    { key: "hosts" as const, label: "Hosts", icon: <Server className="h-3 w-3" /> },
    { key: "snippets" as const, label: "Snippets", icon: <Code className="h-3 w-3" /> },
    { key: "history" as const, label: "History", icon: <Clock className="h-3 w-3" /> },
    { key: "actions" as const, label: "Actions", icon: <Zap className="h-3 w-3" /> },
  ];
      ...
      <DialogContent className="max-w-xl p-0 gap-0 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t bg-muted/30 text-xs text-muted-foreground">
          <div className="flex items-center gap-4">
            ...
            <span>
              <kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px]">Tab</kbd> Switch tabs
            </span>
            <span>
